       ...
       public function reservationStore(Request $request)
{
    ...
    try {
        ...
        $reservation = DB::transaction(function () use ($request, $date, $time) {
            ...
            // Validate business hours (10:00 - 18:00)
            $timeInMinutes = (int)date('H', strtotime($time)) * 60 + (int)date('i', strtotime($time));
            if ($timeInMinutes < 600 || $timeInMinutes > 1080) { // 10:00 = 600, 18:00 = 1080
                throw ValidationException::withMessages([
                    'datetime' => ['営業時間は10:00〜18:00です。'],
                ]);
            }
...
public function getReservationsByDate(Request $request)
{
    ...
    $openEnd = 18 * 60 + 0;     // 18:00 in minutes
